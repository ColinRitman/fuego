...
namespace CryptoNote
{
	namespace parameters
	{
        ...
        const uint64_t CRYPTONOTE_SUBADDRESS_BASE58_PREFIX = CRYPTONOTE_PUBLIC_ADDRESS_BASE58_PREFIX;
    ...
    const unsigned EMISSION_SPEED_FACTOR_FUEGO = 20;   //major version 9
		...
		const uint64_t ALIAS_REGISTRATION_FEE = UINT64_C(10000000);  /* 1 XFG (COIN) */
		...
		const uint64_t MIN_TX_MIXIN_SIZE_V2                          = 2;  // Legacy mixin for BMV7-BMV9
        ...
        // Epoch duration (for fee pool accumulation)
        const uint64_t EPOCH_DURATION_BLOCKS = 900;            // Mainnet: 900 blocks (~5 days)
        const uint64_t TESTNET_EPOCH_DURATION_BLOCKS = 10;     // Testnet: 10 blocks

        // CD Fee Pool constants
        const uint64_t SWAP_FEE_RATE_BPS      = 100;           // 1% (100 basis points)
        ...
        const uint64_t FEE_POOL_RATE_PRECISION = 1000000ULL;   // 1e6 fixed-point

        // Swap fee split: 90% CD yield / 10% Fuego Treasury
        const uint64_t FEE_POOL_CD_SHARE_BPS      = 9000;      // 90%
        const uint64_t FEE_POOL_TREASURY_SHARE_BPS = 1000;     // 10%

        // CD Transfer minimum term
