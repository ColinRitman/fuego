...
namespace CryptoNote
{
  ...
  bool parseTransactionExtra(const std::vector<uint8_t> &transactionExtra, std::vector<TransactionExtraField> &transactionExtraFields)
  {
    ...
    try
    {
      ...
      while (!iss.endOfStream())
      {
        ...
        case TX_EXTRA_FUEGO_CD:
        {
          // 0xFD: Fuego CD (Certificate of Deposit — on-chain yield from swap fee pool)
          // Format: [amount: 8 LE] [term: 4 LE]
          TransactionExtraFuegoCd cd;
          cd.amount = 0;
          for (int i = 0; i < 8; ++i) {
            cd.amount |= static_cast<uint64_t>(read<uint8_t>(iss)) << (i * 8);
          }
          cd.term = 0;
          for (int i = 0; i < 4; ++i) {
            cd.term |= static_cast<uint32_t>(read<uint8_t>(iss)) << (i * 8);
          }
          transactionExtraFields.push_back(cd);
          break;
        }

        default:
          // Unknown tag, skip it or return error?
          // For forward compatibility, we might want to skip it if we knew the length.
          // But most tags don't have a standardized length field.
          return false;
        }
      }
    }
  ...
  struct ExtraSerializerVisitor : public boost::static_visitor<bool>
  {
    ...
    bool operator()(const TransactionExtraFuegoCd &t)
    {
      return addFuegoCdToExtra(extra, t);
    }
  };
...
#pragma pack(pop)
  ...
  bool TransactionExtraFuegoCd::serialize(ISerializer& s) {
    s(amount, "amount");
    s(term, "term");
    return true;
  }
  ...
  bool addFuegoCdToExtra(std::vector<uint8_t>& tx_extra, const TransactionExtraFuegoCd& cd) {
    tx_extra.push_back(TX_EXTRA_FUEGO_CD);
    // Amount (8 bytes LE)
    uint64_t amount = cd.amount;
    for (int i = 0; i < 8; ++i) {
      tx_extra.push_back(static_cast<uint8_t>(amount & 0xFF));
      amount >>= 8;
    }
    // Term (4 bytes LE)
    uint32_t term = cd.term;
    for (int i = 0; i < 4; ++i) {
  ...
  bool getFuegoCdFromExtra(const std::vector<uint8_t>& tx_extra, TransactionExtraFuegoCd& cd) {
    ...
    for (const auto& f : fields) {
      if (f.type() == typeid(TransactionExtraFuegoCd)) {
        cd = boost::get<TransactionExtraFuegoCd>(f);
        return true;
      }
    }
    return false;
