...
func main() {
	...
	for i := 1; i < len(os.Args); i++ {
		...
		case "--help", "-h":
			...
			fmt.Println("  --wallet, -w    Fuego wallet RPC (default: none)")
			fmt.Println("  --testnet       Use testnet defaults (daemon :28280, wallet :28282)")
			fmt.Println()
			fmt.Println("Automation:")
			fmt.Println("  --start-afk     Enable AFK mode automation on startup")
			fmt.Println()
			fmt.Println("Display:")
			fmt.Println("  --pair, -p      Starting pair: sol, eth, xmr, heat, lusd (default: sol)")
